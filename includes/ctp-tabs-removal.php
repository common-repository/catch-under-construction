...
if ( ! function_exists( 'ctp_switch' ) ) {
	...
	function ctp_switch() {
		...
		if ( ! check_ajax_referer( 'ctp_tabs_nonce', 'security', false ) ) {
			wp_die( esc_html__( 'Invalid Nonce', 'catch-under-construction' ) );
		} else {
			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( esc_html__( 'Permission denied!', 'catch-under-construction' ) );
			}
			...
			if ( update_option( 'ctp_options', $option_value ) ) {
				...
				esc_html_e( 'Connection Error. Please try again.', 'catch-under-construction' );
			}
		}
